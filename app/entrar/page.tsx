...
function EntrarPageInner() {
  ...
  // Preservado por quem chegou aqui querendo terminar uma ação específica —
  // hoje só a candidatura pública usa isso (ver app/vagas/[id]/page.tsx),
  // mas qualquer link "entre pra continuar" pode se aproveitar do mesmo campo.
  ...
  async function entrar() {
    ...
    try {
      ...
      const destino = next || (contaRole === 'CLINICA' ? '/clinica' : contaRole === 'PROFISSIONAL' ? '/profissional' : '/admin');
      router.push(destino);
    } catch (err) {
        ...
        <div className="login-brand-inner relative z-10 w-full max-w-[460px] mx-auto flex flex-col gap-4 flex-none items-center
          ...
          <div className="login-copy-group flex flex-col gap-4 items-center md:items-start md:gap-[26px]">
            <div className="flex flex-col items-center gap-4 md:items-start md:gap-3.5">
              <a href="/" className="w-[136px] h-[136px] md:w-[84px] md:h-[84px] md:min-[1600px]:w-24 md:min-[1600px]:h-24 shrink-0 flex items-center justify-center">
                <img
                  src="/logo.svg"
                  alt="ConectVet"
      ...
      >
        <div className="w-full max-w-[380px] md:min-[1600px]:max-w-[420px]">
          ...
          <div className="text-center mt-5 text-[13.5px] text-gray-500">
            Não tem conta? <a href={`/cadastro${next ? `?next=${encodeURIComponent(next)}` : ''}`} className="font-bold hover:underline">Criar conta</a>
          </div>
        </div>
      </div>
    </div>
